...
RemoteMethod.prototype._post = function _post(data, nrHeaders, callback) {
  ...
  if (logger.traceEnabled()) {
    const payload = Buffer.isBuffer(data) ? 'Buffer ' + data.length : data
    logger.trace({ data: payload, compressed: options.compressed }, 'Calling %s on collector API', this.name)
  }
