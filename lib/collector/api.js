...
/**
 * Polls the `get_agent_commands` endpoint for new agent commands queued by the
 * collector for this agent run.
 *
 * @param {Function} callback Runs with `(error, response)` where
 * `response.payload` is a list of `[id, { name, arguments }]` command pairs.
 */
CollectorAPI.prototype.getAgentCommands = function getAgentCommands(callback) {
  const runId = this._agent.config.run_id
  this._runLifecycle(this._methods.get_agent_commands, [runId], callback)
}
