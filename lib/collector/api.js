...
/**
 * Posts the results of executed agent commands to the `agent_command_results`
 * endpoint.
 *
 * @param {object} results Map of command id to result map (or JSON exception).
 * @param {Function} callback Runs with `(error, response)`; the `response` is a
 * `CollectorResponse` whose `retainData` indicates whether unsent results should
 * be retried on the next harvest.
 */
CollectorAPI.prototype.sendCommandResults = function sendCommandResults(results, callback) {
  const runId = this._agent.config.run_id
  this._runLifecycle(this._methods.agent_command_results, [runId, results], callback)
}
...
function _handleResponseCode(response, endpoint, cb) {
  ...
  if (SUCCESS.has(code)) {
    ...
    // If the collector piggy-backed agent commands onto this response, process
    // them in-band rather than relying on the polling loop.
    if (Array.isArray(response.agentActions) && this._agent.commandParser) {
      this._agent.commandParser
        .processInbandCommands(response.agentActions)
        .catch((err) => {
          logger.warn(err, 'Error processing in-band agent commands from %s.', endpoint)
        })
    }

    setImmediate(cb, null, CollectorResponse.success(payload))
