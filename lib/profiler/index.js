...
// for injecting trace/span id
// const kSampleCount = time.constants.kSampleCount
// const DEFAULT_CONTEXT = { current: {} }
...
function initialize(api) {
  ...
  /*
   * This is code to inject trace/span id into cpu profiling data
  const { AsyncLocalStorage } = require('node:async_hooks')

  const run = AsyncLocalStorage.prototype.run
  // should apply to `sample.label` array, need to dig more as it is not working
  AsyncLocalStorage.prototype.run = function (store, callback, ...args) {
    const origCb = callback
    function wrappedCb(...cbArgs) {
      const sampleCount = profiler?.state?.[kSampleCount]
      if (profiler.started) {
        if (sampleCount !== profiler?.lastSampleCount) {
          profiler.lastSampleCount = sampleCount
          profiler.ctx = DEFAULT_CONTEXT
          time.setContext(profiler.ctx)
        }

        const ctx = agent.tracer.getContext()
        if (ctx?.transaction || ctx?.segment) {
          profiler.ctx.current = { 'trace.id': ctx?.transaction?.traceId, 'span.id': ctx?.segment?.id }
        }
      }
      return origCb.apply(this, cbArgs)
    }
    const ret = run.call(this, store, wrappedCb, ...args)
    const sampleCount = profiler?.state?.[kSampleCount]
    if (profiler.started) {
      if (sampleCount !== profiler?.lastSampleCount) {
        profiler.lastSampleCount = sampleCount
      ...
      const ctx = agent.tracer.getContext()
      if (ctx?.transaction || ctx?.segment) {
        profiler.ctx.current = { traceId: ctx?.transaction?.traceId, spanId: ctx?.segment?.id }
      }
    }
    return ret
  }
...
Profiler.prototype.start = async function start() {
  ...
  if (this.selectedProfiles & PROFILING_TYPE_CPU) {
    ...
    time.start({
      ...
      durationMillis: 60 * 1e3,
      // withContexts: true,
      collectCpuTime: true
    })
    /*
     * trace/span id injection code
    this.state = time.getState()
    this.ctx = DEFAULT_CONTEXT
    time.setContext(this.ctx)
    */
  }
...
/*
 * trace/span id injection code
Profiler.prototype.linkContext = function linkContext({ node, context }) {
  if (!context?.context?.current) {
    return {}
  }

  logger.trace('Linking context to CPU sample: %j', context.context.current)
...
Profiler.prototype._collectCpuSamples = async function _collectCpuSamples() {
  ...
  // const profile = time.stop(true, this.linkContext)
...
/*
 * trace/span id injection code
function assignLabels({ attrs, sample, profile }) {
  for (let i = 0; i < sample.label.length; i++) {
    const key = _getString(profile, sample.label[i].key)
    const value = _getString(profile, sample.label[i].str)
    attrs[key] = value
  }
}
*/
...
function _recordSamples(
  ...
  for (let i = 0; i < profile.sample.length; i++) {
    const sample = profile.sample[i]

    // for injection trace/span ids in profiling info
    // assignLabels({ attrs, sample, profile })

    attrs['sample_seq'] = i

    for (let j = 0; j < sample.value.length; j++) {
