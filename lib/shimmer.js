...
const shimmer = (module.exports = {
  ...
  reinstrument: function reinstrument(agent, modulePath) {
    // TODO: abstract this into a util
    const resolvedName = require.resolve(modulePath)
    const parsed = parse(resolvedName)
    return _postLoad(agent, require(modulePath), modulePath, parsed.basedir)
  },
   ...
   */
  isInstrumented(nodule, moduleName) {
  ...
  instrumentPostLoad(agent, module, moduleName, resolvedName, returnModule = false) {
    ...
    return returnModule ? result : shimmer.isInstrumented(module, moduleName)
  }
})
...
function instrumentPostLoad(agent, nodule, moduleName, resolvedName, esmResolver) {
  ...
  instrumentations.forEach((instrumentation, index) => {
    const isInstrumented = nodule[symbols.instrumented].includes(index)
    const failedInstrumentation = nodule[symbols.instrumentedErrored].includes(index)
    if (isInstrumented || failedInstrumentation) {
      const msg = isInstrumented ? 'Already instrumented' : 'Failed to instrument'
      logger.trace(`${msg} ${resolvedName} ${moduleName}, skipping registering instrumentation`)
      return nodule
    }
    ...
    if (instrumentation.type === MODULE_TYPE.TRACKING) {
      nodule[symbols.instrumented].push(index)
      return nodule
    }
 ...
 */
function loadInstrumentation({ shim, resolvedNodule, moduleName, nodule, index, instrumentation }) {
  try {
    ...
    nodule[symbols.instrumentedErrored].push(index)
...
function _postLoad(agent, nodule, name, resolvedName, esmResolver) {
  ...
  if (hasPostLoadInstrumentation) {
    addInstrumentationSymbols(nodule)
...
function addInstrumentationSymbols(nodule) {
  if (!nodule[symbols.instrumented]) {
    nodule[symbols.instrumented] = []
  }

  if (!nodule[symbols.instrumentedErrored]) {
    nodule[symbols.instrumentedErrored] = []
  }
}
