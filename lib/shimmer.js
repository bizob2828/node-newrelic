...
const shimmer = (module.exports = {
  ...
  patchModule: function patchModule(agent) {
    ...
    /*
    debugger
    shimmer.wrapMethod(Module.prototype, 'Module.prototype', 'require', function wrapRequire(orig) {
      return function wrappedRequire(id) {
        const filename = Module._resolveFilename(id, this)
        const isBuiltin = filename.indexOf(path.sep) === -1
        //console.log('requiring id', id, filename, isBuiltin, parse(filename))
        debugger
    ...
    */
    ...
    shimmer.wrapMethod(Module, 'Module', '_resolveFilename', function wrapRes(resolve) {
      return function wrappedResolveFilename(file) {
        ...
        // matchupName.call(this, resolvedFilepath, file)
        ...
        if (filepathMap[file] !== resolvedFilepath) {
          ...
          // TODO: update logic to pass in the parsed module from path as 2nd arg
          // it appears we cannot instrument require because it won't fire in ESM
    ...
    shimmer.wrapMethod(Module, 'Module', '_load', function wrapLoad(load) {
      return function wrappedLoad(request, parent, isMain) {
        ...
        // matchupName.call(this, fileName, request)
        // TODO: update logic to pass in the parsed module from path as 3rd arg instead of request
        return _postLoad(agent, m, request, fileName)
      }
    })

    /*
    function matchupName(fileName, request) {
      const n = parse(fileName)
      //console.log('found file name', fileName, request, n)
      if (n) {
        debugger
        const paths = Module._resolveLookupPaths(n.name, this, true)
        if (paths) {
          const res = Module._findPath(n.name, [n.basedir, ...paths])
          console.log('ne paths', res, request, fileName, n.name + path.sep + path.relative(n.basedir, fileName))
        }
      }
    }
    */
  ...
  getInstrumentationNameFromModuleName(moduleName) {
    if (path.isAbsolute(moduleName)) {
      const pkgInfo = parse(moduleName)
      if (pkgInfo && pkgInfo.path !== 'package.json') {
        const paths = Module._resolveLookupPaths(pkgInfo.name, this, true)
        const res = Module._findPath(pkgInfo.name, [pkgInfo.basedir, ...paths])
        console.log(pkgInfo)
        console.log(
          'ne paths',
          res,
          moduleName,
          pkgInfo.name + path.sep + path.relative(pkgInfo.basedir, moduleName)
        )
        return pkgInfo.name
      }
    }
