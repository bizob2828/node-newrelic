...
const shimmer = (module.exports = {
  ...
  registerLegacyEsmHooks(waitForAllMessagesAcknowledged) {
    this._waitForEsmAck = waitForAllMessagesAcknowledged
    this._iitm = new IitmHook(pkgsToHook, function onESMHook(exports, name, basedir) {
      debugger
      return _postLoad(globalAgent, exports, name, basedir, true)
    })
  },

  /**
   * Waits for the import-in-the-middle hook.mjs loader to acknowledge all
   * dynamically added packages. Call this after registering instrumentation
   * and before importing the module to ensure the ESM loader will intercept it.
   *
   * @returns {Promise}
   */
  waitForAllEsmHooksAcknowledged() {
    return this._waitForEsmAck ? this._waitForEsmAck() : Promise.resolve()
  },

  registerHooks(agent) {
    // include the tracking packages, we should prob get rid of this
  ...
  registerInstrumentation: function registerInstrumentation(opts) {
    ...
    if (!registeredInstrumentation) {
      ...
      if (!pkgsToHook.includes(pkgHook)) {
        ...
        // If the ESM hook is already active, the hook.mjs loader's includeModules
        // list was already set (to whatever was in pkgsToHook at that time). We
        // need to notify the loader about this new package so it will intercept it.
        // Creating a new IitmHook triggers sendModulesToLoader internally.
        if (shimmer._iitm) {
          debugger
          new IitmHook([pkgHook], () => {})
        }
      }
    }
