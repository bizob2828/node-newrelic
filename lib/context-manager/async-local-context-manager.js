...
class AsyncLocalContextManager {
  ...
  setContext(newContext) {
    const ctx = new Context(newContext)
  ...
  runInContext(context, callback, cbThis, args) {
    ...
    ctx = ctx.updateContext(context)
    const toInvoke = cbThis ? callback.bind(cbThis) : callback

    if (args) {
      return this._asyncLocalStorage.run(ctx, toInvoke, ...args)
    }

    return this._asyncLocalStorage.run(ctx, toInvoke)
  }
}
