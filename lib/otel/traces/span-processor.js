...
const hrTimeToMilliseconds = require('../utils/hr-to-millis.js')
...
module.exports = class NrSpanProcessor {
  ...
  finalizeTransaction({ rule, segment, span, transaction }) {
    ...
    // touch the root to mimic our core instrumentation behavior
    // otherwise the trace root will be named `Truncated/ROOT`
    transaction.trace.root.touch()
    // End the corresponding transaction for the entry point server span.
    // We do that when the span ends to ensure all data has been processed
