...
class SpanEvent {
  ...
  static fromSegment({ segment, transaction, parentId = null, isRoot = false, inProcessSpans, partialGranularityMode }) {
    ...
    let exitAttrs = []
    if (partialGranularityMode) {
      if (!(entryPoint || SpanEvent.isLlm(segment))) {
        const attrKeys = Object.keys(attributes)
        exitAttrs = SPAN_ENTITY_RELATIONSHIP_ATTRIBUTES.filter((item => attrKeys.includes(item)))
        if (exitAttrs.length < 1) {
          return
        }
      }
    }
