...
const { CommandParser } = require('./agent-commands')
...
function Agent(config) {
  ...
  // Server-to-agent control channel. Polls for and executes agent commands each
  // harvest cycle when enabled.
  this.commandParser = new CommandParser({ agent: this })
  this._commandParserTimer = null
...
Agent.prototype.onConnect = function onConnect(shouldImmediatelyHarvest, callback) {
  ...
  this._startCommandParser()
  this.setState('started')
}

/**
 * Starts polling for agent commands on the harvest interval when the command
 * parser is enabled. The timer is unref'd so it does not keep the process alive.
 */
Agent.prototype._startCommandParser = function _startCommandParser() {
  if (!this.commandParser.enabled || this._commandParserTimer) {
    return
  }

  this._commandParserTimer = setInterval(() => {
    this.commandParser.runCommands().catch((err) => {
      logger.warn(err, 'Agent command execution loop failed.')
    })
  }, DEFAULT_HARVEST_INTERVAL_MS)
  this._commandParserTimer.unref()
}

/**
 * Stops the agent command polling loop.
 */
Agent.prototype._stopCommandParser = function _stopCommandParser() {
  if (this._commandParserTimer) {
    clearInterval(this._commandParserTimer)
    this._commandParserTimer = null
  }
}
