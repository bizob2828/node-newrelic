...
/* eslint sonarjs/cognitive-complexity: ["error", 79] -- TODO: https://issues.newrelic.com/browse/NEWRELIC-5252*/
...
function recordSubscribedConsume(nodule, properties, spec) {
  ...
  function makeWrapConsumer(queue, destinationName) {
    ...
    // Allows for users to do something with the promise rejection
    // Normally you would call `noticeError`
    msgDescDefaults.handlePromiseRejection = msgDescDefaults.handlePromiseRejection || false
        ...
        function createConsumeTrans() {
          ...
          try {
            ...
            if (shim.isPromise(ret)) {
              ...
              ret = shim.interceptPromise(ret, endTransaction, endFailedTransaction)
           ...
           * @param err
           */
          function endFailedTransaction(err) {
            if (typeof msgDesc.handlePromiseRejection === 'function') {
              msgDesc.handlePromiseRejection(err)
            }
