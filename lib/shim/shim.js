...
function recordWrapper({ shim, fn, name, recordNamer }) {
  return function wrapper() {
    ...
    if (segDesc.parent) {
      segDesc.parent = segDesc.parent?.transaction?.isActive() && segDesc.parent?.segment ? segDesc.parent.segment : shim.getActiveSegment() 
      transaction = segDesc?.parent?.transaction
    ...
    const segment = shouldCreateSegment ? _rawCreateSegment(shim, segDesc) : segDesc.parent
...
function _applyRecorderSegment({ transaction, segment, ctx, args, segDesc, shim, fn, name }) {
  ...
  let ret
  try {
    ret = shim.applySegment(fn, { transaction, segment }, true, ctx, args, segDesc.inContext)
...
function bindSegment(nodule, property, segment, full) {
  ...
  return this.wrap(nodule, property, function wrapFunc(shim, func) {
    ...
    transaction = transaction || shim.tracer.getTransaction()
    if (!segment) {
      return func
    }
...
function wrapCallback({ shim, args, cbIdx, parentSegment, spec }) {
  ...
  const ctx = shim.agent._contextManager.getContext()
  ...
  args[cbIdx] = shim.wrap(cb, null, function callbackWrapper(shim, fn, name) {
    return function wrappedCallback() {
      ...
      return shim.applySegment(cb, { transaction, segment }, true, this, arguments)
    }
  })
    ...
    return function wrappedEmit(evnt) {
      ...
      if (evnt === specEvent && tx.isActive()) {
        if (!eventBoundEmit) {
          ...
          eventBoundEmit = shim.bindSegment(emit, { transaction: tx, segment: eventSegment }, true)
        }
