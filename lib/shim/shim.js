 ...
 * @param {Function} resolvedCb - A function to call when the promise is resolved.
 * @param {Function} [rejectedCb=resolvedCb] - A function to call when the promise is rejected.
 * @returns {Promise} A new promise to replace the original one.
 */
function interceptPromise(prom, resolvedCb, rejectedCb = resolvedCb) {
  prom.then(resolvedCb, rejectedCb)
  return prom
}
