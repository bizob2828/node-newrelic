...
function recordWrapper({ shim, fn, name, recordNamer }) {
  return function wrapper() {
    ...
    if (segDesc.parent) {
      ...
      // TODO: we should check the otel ctx to see if an otel span is the parent
...
function getSegment(obj) {
  ...
  return this._contextManager.getSegment()
}
...
function applySegment(func, segment, full, context, args, inContextCB) {
  ...
  // TODO: Should we check the otel ctx to see if the parent id is diff? 
  const prevSegment = contextManager.getSegment()
