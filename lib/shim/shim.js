...
function wrap(nodule, properties, spec, args) {
  ...
  return _wrap(this, nodule, properties, spec, args)
}
 ...
 */
function _wrap(shim, original, properties, spec, args) {
  const wrappedProps = new Map()
  // If we're just wrapping one thing, just wrap it and return.
  if (properties == null) {
    properties = { name: shim.getName(original), xport: original }
    shim.logger.trace('Wrapping nodule itself (%s).', properties.name)
  }

  // Coerce properties into an array.
  if (!shim.isArray(properties)) {
    properties = [properties]
  }

  // Wrap each property and return the nodule.
  shim.logger.trace('Wrapping %d properties on nodule.', properties.length)
  properties.forEach((property) => {
    ...
    if (typeof property === 'object') {
      ...
      name = property
      xport = original[property]
    } 

    if (!xport || shim.isString(xport)) {
      shim.logger.debug('Not wrapping missing property "%s"', name)
      return
    }

    // Assemble the spec's arguments.
    const specArgs = [shim, xport, name]
    ...
    if (wrapped && wrapped !== original) {
      ...
      wrappedProps.set(name, wrapped)
    }
  })

  if (!wrappedProps.size) {
    shim.logger.debug('Did not wrap any properties on xport')
    return original
  }
  ...
  const handler = {
    get(target, prop) {
      if (prop === symbols.original) {
        return target
      } else if (prop === symbols.wrapped) {
        const wrapped = wrappedProps.get(target.name)
        if (wrapped) {
          return wrapped[symbols.wrapped]
        }
      } else if (wrappedProps.has(prop)) {
        return wrappedProps.get(prop)
      }
