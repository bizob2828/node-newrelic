...
function recordWrapper({ shim, fn, name, recordNamer }) {
  return function wrapper() {
    ...
    if (segDesc.parent) {
      transaction = segDesc?.parent?.transaction || shim.tracer.getTransaction()
      segDesc.parent =
        segDesc.parent?.transaction?.isActive() && segDesc.parent?.segment
          ? segDesc.parent.segment
          : shim.getActiveSegment()
      // We only want to continue recording in a transaction if the
      // transaction is active.
    } else {
      segDesc.parent = shim.getActiveSegment()
      transaction = shim.tracer.getTransaction()
    }

    if (!(segDesc.parent && transaction)) {
    ...
    const shouldCreateSegment = !(
      segDesc.parent.opaque ||
      (segDesc.internal && segDesc.parent.internal && shim === segDesc.parent.shim)
    )

    const segment = shouldCreateSegment
      ? _rawCreateSegment(shim, segDesc)
      : segDesc.parent
 ...
 */
function _doRecord({ segment, transaction, args, segDesc, shouldCreateSegment, shim, fn, name }) {
 ...
 */
function _applyRecorderSegment({ transaction, segment, ctx, args, segDesc, shim, fn, name }) {
  let error = null
  let promised = false
  let ret
  try {
...
function bindSegment(nodule, property, segment, full) {
  ...
  if (segment?.segment instanceof TraceSegment) {
  ...
  return this.wrap(nodule, property, function wrapFunc(shim, func) {
    ...
    const ctx = shim.tracer.getContext({ segment })
    const binder = _makeBindWrapper(shim, func, ctx, full || false)
...
function wrapCallback({ shim, args, cbIdx, parentSegment, spec }) {
  ...
  let ctx = shim.tracer.getContext()
  ...
  args[cbIdx] = shim.wrap(cb, null, function callbackWrapper(shim, fn, name) {
    return function wrappedCallback() {
      ...
      if (segment) {
        segment.async = false
        ctx = shim.tracer.getContext({ segment })
      }
      ...
      return shim.applySegment(cb, ctx, true, this, arguments)
    }
  })
...
function applySegment(func, segment, full, context, args, inContextCB) {
  ...
  if (!(segment instanceof TraceSegment || segment?.segment instanceof TraceSegment)) {
  ...
  const ctx = this.tracer.getContext({ segment: active })
  return this.tracer.bindFunction(runInContextCb, ctx, full).apply(context, args)
}
 ...
 */
function _makeBindWrapper(shim, fn, ctx, full) {
  return function wrapper() {
    return shim.applySegment(fn, ctx, full, this, arguments)
  }
}
...
function wrapStreamEmit({ stream, shim, segment, specEvent, shouldCreateSegment, segmentName }) {
  ...
  shim.wrap(stream, 'emit', function wrapEmit(shim, emit) {
    ...
    debugger
    //const streamBoundEmit = shim.bindSegment(emit, { transaction: tx, segment }, true)
    ...
    return function wrappedEmit(evnt) {
      ...
      if (evnt === specEvent && tx.isActive()) {
        if (!eventBoundEmit) {
          ...
          debugger
          const ctx = shim.tracer.getContext({ segment: eventSegment })
          eventBoundEmit = shim.bindSegment(emit, ctx, true)
        }
