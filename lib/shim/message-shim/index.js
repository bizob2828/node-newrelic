...
const messageRecorder = require('../../metrics/recorders/message')
...
function recordProduce(nodule, properties, recordNamer) {
  ...
  return this.record(nodule, properties, function recordProd(shim) {
    ...
    const namer = _nameMessageSegment(shim, msgDesc, shim._metrics.PRODUCE)
    msgDesc.name = namer.name
    ...
    msgDesc.recorder = messageRecorder.bind(null, namer.rollupMetric)
    return msgDesc
  })
}
/**
...
function recordPurgeQueue(nodule, properties, spec) {
  ...
  return this.record(nodule, properties, function purgeRecorder(shim, fn, name, args) {
    ...
    const namer  = _nameMessageSegment(
      shim,
      {
    ...
    descriptor.name = namer.name
    descriptor.recorder = messageRecorder.bind(null, namer.rollupMetric)
    return descriptor
  })
}
