...
for (const [dsName, dsType] of Object.entries(DATASTORE_NAMES)) {
  Shim.defineProperty(DatastoreShim, dsName, dsType)
  Shim.defineProperty(DatastoreShim.prototype, dsName, dsType)
}

// Add constants on the shim for the provided query parsers.
DatastoreShim.QUERY_PARSERS = QUERY_PARSERS
for (const [qpName, qpType] of Object.entries(QUERY_PARSERS)) {
  Shim.defineProperty(DatastoreShim, qpName, qpType)
  Shim.defineProperty(DatastoreShim.prototype, qpName, qpType)
}
...
function recordOperation(nodule, properties, opSpec) {
  ...
  return this.record(nodule, properties, function operationRecorder(shim, fn, fnName, args) {
    ...
    if (!hasOwnProperty(segDesc, 'record') || segDesc.record !== false) {
      ...
      const baseOperationRecorder = recordOperationMetrics.bind(shim)
      if (securityChannels.nosqlOperationStart.hasSubscribers || securityChannels.nosqlOperationFinish.hasSubscribers) {
        const transaction = shim.agent.tracer.getTransaction()
        const dbSystem = (shim._datastore || 'unknown').toLowerCase()
        const params = segDesc.parameters || {}
        const operation = fnName

        if (transaction && securityChannels.nosqlOperationStart.hasSubscribers) {
          securityChannels.nosqlOperationStart.publish({
            dbSystem,
            library: shim._datastore || 'unknown',
            operation,
            collection: params.collection || segDesc.name || null,
            filter: null,
            host: params.host || null,
            port: params.port_path_or_id || null,
            database: params.database_name || null,
            transaction,
            segment: shim.getActiveSegment()
          })
        }

        segDesc.recorder = function nosqlRecorderWithSecurity(segment, tx, recName) {
          baseOperationRecorder.call(this, segment, tx, recName)
          if (securityChannels.nosqlOperationFinish.hasSubscribers) {
            securityChannels.nosqlOperationFinish.publish({
              dbSystem,
              operation,
              error: null,
              transaction: tx,
        ...
        segDesc.recorder = baseOperationRecorder
      }
    }
...
function _recordQuery(suffix, nodule, properties, querySpec) {
  ...
  return this.record(nodule, properties, function queryRecorder(shim, fn, fnName, args) {
    ...
    if (!hasOwnProperty(segDesc, 'record') || segDesc.record !== false) {
      ...
      const baseRecorder = recordQueryMetrics.bind(parsed)
      if (securityChannels.sqlQueryStart.hasSubscribers || securityChannels.sqlQueryFinish.hasSubscribers) {
        const transaction = shim.agent.tracer.getTransaction()
        const dialect = (shim._datastore || 'unknown').toLowerCase()
        const params = segDesc.parameters || {}

        if (transaction && securityChannels.sqlQueryStart.hasSubscribers) {
          securityChannels.sqlQueryStart.publish({
            sql: queryStr,
            dialect,
            params: null,
            host: params.host || null,
        ...
        segDesc.recorder = function sqlRecorderWithSecurity(segment, tx, recName) {
          baseRecorder.call(this, segment, tx, recName)
          if (securityChannels.sqlQueryFinish.hasSubscribers) {
            securityChannels.sqlQueryFinish.publish({
              sql: queryStr,
              dialect,
              error: null,
              rowCount: null,
