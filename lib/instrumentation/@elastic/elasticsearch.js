  ...
  shim.recordQuery(elastic.Transport.prototype, 'request', function wrapQuery(shim, _, __, args) {
    ...
    return {
      ...
      inContext: function inContext() {
        getConnection.call(ctx, shim, pkgVersion)
      }
    }
  })
}
 ...
 */
function getConnection(shim, pkgVersion) {
  debugger
  if (semver.gte(pkgVersion, '8.0.0')) {
    ...
    const [connection] = this.connectionPool.connections.entries().next().value
    const url = new URL(connection)
    return shim.captureInstanceAttributes(`${url.protocol}//${url.hostname}`, url.port)
  }
}
