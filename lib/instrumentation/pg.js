...
const NR_PG_CONN = Symbol('nr_conn')
...
module.exports = function initialize(agent, pgsql, moduleName, shim) {
  ...
  function wrapJSClientQuery(shim, _, __, queryArgs) {
    ...
    const args = ['pg.ConnectionQueye', activeSegment]
    const connQueue = shim.createSegment(...args)
    this.connection[NR_PG_CONN] = connQueue
    connQueue.start()
  ...
  // wrapping for JS
  shim.wrap(pgsql && pgsql.Connection && pgsql.Connection.prototype, 'query', function wrapJsConnQuery(shim, query) {
    return function wrappedConQuery() {
      debugger
      const activeSegment = shim.getActiveSegment()
      activeSegment.timer.state = 1
      activeSegment.timer.begin()
      const segment = this[NR_PG_CONN]
      segment.end()
