  ...
  function wrapListen2(shim, fn) {
    return function wrappedListen2() {
      ...
      function wrappedEmit(ev, socket) {
        ...
        const orig = socket._handle.onread
        /*if (socket._handle.onread) {
          debugger
          const ctx = shim.tracer.getContext({ segment: child })
          shim.tracer.bindFunction(socket._handle.onread, ctx)
          //shim.bindSegment(socket._handle, 'onread', ctx)
        }
        */

        const ret = shim.applySegment(emit, { segment: child }, true, this, arguments)
        let ctx = shim.tracer.getContext()
        socket._handle.onread = function wrapped() {
          debugger
          ctx = ctx.addSegment(child)
          debugger
          return shim.tracer.bindFunction(orig, ctx).apply(this, arguments)
        }
        debugger
        return ret
      }
