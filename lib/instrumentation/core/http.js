...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    if (securityChannels.httpInboundStart.hasSubscribers) {
      let query = {}
      try {
      ...
      } catch (_e) { /* invalid URL, skip query parsing */ }
      securityChannels.httpInboundStart.publish({
        req: request,
        res: response,
        transaction,
        url: absoluteUrl,
...
function instrumentedFinish(transaction) {
  ...
  if (securityChannels.httpInboundFinish.hasSubscribers) {
    securityChannels.httpInboundFinish.publish({
      req: this.req,
      res: this,
      transaction,
