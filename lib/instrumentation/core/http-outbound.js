...
const SECURITY_URL = Symbol('nr.securityOutboundUrl')
...
/**
 * Injects DT headers, creates segment for outbound http request.
...
function instrumentRequest(
  ...
  maybeAddDtHeaders(transaction, outboundHeaders, opts?.headers)
  ...
  if (securityChannels.httpOutboundStart.hasSubscribers) {
    const outboundUrl = `${opts.protocol || 'http:'}//${hostname}${opts.path || '/'}`
    segment[SECURITY_URL] = outboundUrl
    securityChannels.httpOutboundStart.publish({
      url: outboundUrl,
...
/**
 * It will add DT headers to the outgoing headers, if enabled
 *
 ...
 */
function maybeAddDtHeaders(transaction, outboundHeaders, headers = {}) {
  // TODO: remove `symbols.disableDT` check when we drop `aws-sdk@2`
...
function handleResponse({ agent, segment, transaction, host, res }) {
  ...
  shimmer.wrapMethod(res, 'response', 'emit', function wrapEmit(emit) {
    ...
    return function wrappedResponseEmit(evnt) {
      if (evnt === 'end') {
        segment.end()
        if (securityChannels.httpOutboundFinish.hasSubscribers) {
          securityChannels.httpOutboundFinish.publish({
            url: segment[SECURITY_URL] || `http://${host}`,
