...
module.exports = function instrumentOutbound(agent, opts, makeRequest) {
  ...
  function instrumentRequest(segment) {
    ...
    segment.start()
    const awsMeta = JSON.parse(opts.headers['x-nr-aws'])
    if (awsMeta) {
      segment.addAttribute('aws.operation', awsMeta.operation)
      segment.addAttribute('aws.service', awsMeta.service)
      segment.addAttribute('aws.region', awsMeta.region)
    }
...
function handleResponse(segment, hostname, req, res) {
  ...
  if (res.headers['x-amzn-requestid']) {
    segment.addAttribute('aws.requestId', res.headers['x-amzn-requestid'])
  }
