...
module.exports = function addUndiciChannels(agent, undici, modName, shim) {
  if (
    ...
    !(
      agent.config.feature_flag.undici_instrumentation ||
      agent.config.feature_flag.instrument_global_fetch
    )
  ) {
    logger.warnOnce(
      'diagnostics_channel or feature_flag.undici_instrumentation = false or feature_flag.instrument_global_fetch = false. Skipping undici/global fetch instrumentation.'
    )
    return
  }
