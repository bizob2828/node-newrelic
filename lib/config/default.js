...
defaultConfig.definition = () => {
  return {
    ...
    distributed_tracing: {
      ...
      /**
       * When `true`, and an outbound message queue produce call is made within a
       * transaction that is not sampled, the agent will emit only the empty
       * `nrns` (New Relic Not Sampled) header and no other trace headers. This
       * signals to the downstream consumer that the upstream transaction was not
       * sampled without the overhead of the full trace context headers.
       */
      send_message_queue_not_sampled_header: {
        formatter: boolean,
        default: false
      },
