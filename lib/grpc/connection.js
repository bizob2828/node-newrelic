...
class GrpcConnection extends EventEmitter {
  ...
  _setupSpanStreamObservers(stream) {
    ...
    stream.on('data', function data(response) {
      if (response?.messages_seen) {
        const value = parseInt(response.messages_seen, 10)
        connection._metrics.getOrCreateMetric(NAMES.INFINITE_TRACING.SENT).incrementCallCount(value)
      }
