 ...
 * @param {string} traceId if present, it will use this to assign traceId of
 * transaction. only used in otel bridge mode to ensure trace id is same as otel
 * spans
...
function acceptDistributedTraceHeaders(transportType, headers) {
  ...
  } else if (NRNS_TRACE_HEADER in headers && this.type === TYPES.MESSAGE) {
    // Only accept the `nrns` header on a new message transaction. When a consumer
    // call creates a span inside an existing transaction rather than a new
    // MessageTransaction, the `nrns` header must be ignored.
    logger.trace('Accepting nrns not-sampled DT signal for transaction %s', this.id)
    this.acceptNrnsPayload(transport)
  }
}

/**
 * Accepts an inbound `nrns` (New Relic Not Sampled) header. This is an empty
 * header indicating the upstream transaction was not sampled and originated from
 * a message queue produce call. The transaction is treated as a parent-not-sampled
 * inbound request with a missing trace id, span id, etc.
 *
 * @param {string} transport - Transport type that delivered the request.
 */
Transaction.prototype.acceptNrnsPayload = acceptNrnsPayload
function acceptNrnsPayload(transport) {
  if (this.isDistributedTrace) {
    logger.warn(
      'Already accepted or created a distributed trace payload for transaction %s, ignoring nrns header',
      this.id
    )
  ...
  // The trace id, parent span id, and parent transaction id are intentionally
  // left unset. There is no upstream trace context, so the transaction generates
  // its own trace id and starts a new trace with a not-sampled decision.
  this.agent.samplers.applyDTSamplingDecision({
    transaction: this,
    traceparent: undefined,
    tracestate: undefined
  })
  this.agent.recordSupportability('TraceContext/Accept/NRNS')
}
 ...
 * @param {boolean} [opts.isMessageQueue] - `true` when the outbound request is a
 * message queue produce call. When combined with the
 * `distributed_tracing.send_message_queue_not_sampled_header` config and a
 * transaction that is not sampled, only the `nrns` header is emitted.
 */
Transaction.prototype.insertDistributedTraceHeaders = insertDistributedTraceHeaders
function insertDistributedTraceHeaders(headers, setter, spanContext, opts = {}) {
  const { isMessageQueue = false } = opts
  if (!headers) {
  ...
  if (!this.agent.config.distributed_tracing.enabled) {
    logger.trace('DT is not enabled, not adding w3c trace context headers')
    return
  }
  ...
  if (
    isMessageQueue &&
    this.agent.config.distributed_tracing.send_message_queue_not_sampled_header &&
    this.isSampled() === false
  ) {
    // For an unsampled message queue produce call, emit only the empty `nrns`
    // header and no other trace headers.
    headers[NRNS_TRACE_HEADER] = ''
    this.isDistributedTrace = true
    this.agent.recordSupportability('TraceContext/Create/NRNS')
    logger.trace('Added outbound nrns not-sampled header in transaction %s', this.id)
    return
  }
 ...
 * @param {string} [params.transport] - Transport type that delivered the request.
 */
Transaction.prototype.addDtHeaders = function addDtHeaders({ headers, transport }) {
  if (!this.agent.config.distributed_tracing.enabled) {
    logger.debug('DT is not enabled, not accepted upstream headers')
    return
  }
