...
function getSegment() {
  const ctx = this.getContext()
  return ctx?.segment || null
}

// TODO: update to setNewContext or something like that
function setSegment({ segment, transaction }) {
  this._contextManager.setContext({ transaction, segment })
}
...
function addSegment(name, recorder, parent, full, task) {
  ...
  return this.bindFunction(task, segment, full)(segment, transaction)
}
...
function transactionProxy(handler) {
  ...
  const wrapped = function wrapTransactionInvocation() {
    ...
    const ctx = { transaction, segment: transaction.trace.root }
...
function transactionNestProxy(type, handler) {
  ...
  const wrapped = function wrapTransactionInvocation() {
    ...
    if (createNew) {
      ...
      segment = { transaction, segment: transaction.trace.root }
    }
...
function bindFunction(handler, segment, full) {
  ...
  return _makeWrapped({ tracer: this, handler, segment, transaction, full: !!full })
}
function _makeWrapped({ tracer, handler, segment, transaction, full }) {
  ...
  function wrapped() {
    ...
    try {
      return tracer._contextManager.runInContext({ transaction, segment }, handler, this, arguments)
    } catch (err) {
