...
const ctx = require('../../async-context')
...
Object.defineProperty(Tracer.prototype, 'segment', {
  get: function segmentGetter() {
    return ctx.active && ctx.active.segment
...
// TODO: this is where we should run AsyncLocalStorage.run
...
function getOriginal(fn) {
  //return ctx.active && ctx.active.original || fn
  return fn && fn[ORIGINAL] ? fn[ORIGINAL] : fn
}

function getSegmentFromWrapped(fn) {
  return ctx.active && ctx.active.segment || null
