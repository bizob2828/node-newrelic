...
const incrementSegments = require('../../util/segments')
...
function getContext({ transaction, segment } = {}) {
  let ctx = this._contextManager.getContext()
  if (transaction) {
    return ctx.addTransaction(transaction)
  }

  if (segment) {
    return ctx.addSegment(segment)
  }
...
function getSpanContext() {
  ...
  return currentSegment?.getSpanContext()
}
...
function addSegment(name, recorder, parent, full, task) {
  ...
  if (segment) {
    ctx = ctx.addSegment(segment)
  }
  return this.bindFunction(task, ctx, full)(segment, transaction)
}
...
function transactionProxy(handler) {
  ...
  const wrapped = function wrapTransactionInvocation() {
    ...
    const currentTx = tracer.getTransaction()
    if (segment) {
      if (currentTx.traceStacks) {
        segment.probe('!!! Nested transaction creation !!!')
        currentTx.traceFlag = true // Will log the stacks when it ends.
      }
      logger.warn(
        {
    ...
    const ctx = tracer.getContext({ transaction })

    return tracer.bindFunction(handler, ctx, true).apply(this, arguments)
  }
...
function transactionNestProxy(type, handler) {
  ...
  const wrapped = function wrapTransactionInvocation() {
    ...
    if (createNew) {
      ...
      ctx = tracer.getContext({ transaction })
    }

    return tracer.bindFunction(handler, ctx).apply(this, arguments)
  }
...
function bindFunction(handler, ctx, full) {
  if (typeof handler !== 'function') {
    return handler
  }

  return _makeWrapped({ tracer: this, handler, ctx, full: !!full })
}
function _makeWrapped({ tracer, handler, ctx, full }) {
  const { segment } = ctx
  ...
  function wrapped() {
    ...
    try {
      return tracer._contextManager.runInContext(ctx, handler, this, arguments)
    } catch (err) {
