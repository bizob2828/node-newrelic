...
function Tracer(agent) {
  ...
  this.agent = agent
  this._contextManager = new AsyncLocalContextManager(agent.config.opentelemetry.enabled)
}
...
/**
 * Create segment and assign recorder to transaction. This also increments
 * counters of segments.
 *
 * Does not create segment if there is no parent, transaction is not active
 * or parent is opaque.
 *
 * @param {object} params to fn
 * @param {string} params.id if present, it will use id as segment.id. only
 * used in otel bridge mode.
 ...
 * @returns {TraceSegment|null} returns new segment, existing parent if opaque
 * or null (no parent or transaction inactive)
 */
...
function runInContext({ handler, context, full, thisArg, args }) {
  ...
  try {
    return this._contextManager.runInContext(context, handler, thisArg, args)
  } catch (err) {
    logger.trace({ error: err }, 'Error from wrapped function:')
    throw err // Re-throwing application error, this is not an agent error.
  } finally {
