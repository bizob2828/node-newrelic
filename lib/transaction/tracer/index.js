...
Object.defineProperty(Tracer.prototype, 'segment', {
  get: function segmentGetter() {
    return this.ctxMgr.active && this.ctxMgr.active.segment
  },
  set: function segmentSetter(segment) {
    const store = this.ctxMgr.active
    if (store) {
...
function _makeWrapped(tracer, handler, active, full) {
  ...
  // TODO: still need this for legacy i think
  if (tracer.agent.config.feature_flag.use_legacy_context) {
  ...
  function wrapped(...args) {
    ...
    try {
      return tracer.ctxMgr.runIn({
        store: {
          segment: active,
          original: handler // TODO update code to use this over the one attached to fn, also check if it is already wrapped and properly attach the right one
        },
        cb: () => {
