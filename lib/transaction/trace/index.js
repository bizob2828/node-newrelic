...
Trace.prototype.generateSpanEvents = function generateSpanEvents() {
  ...
  for (let i = 0; i < segments.length; ++i) {
    ...
    const segmentInfo = new DTTraceNode(segments[i], parentId, isRoot)
...
Trace.prototype.getChildren = function getChildren(id) {
  const childSegments = this.segments.filter((segment) => segment.parentId === id)
  return childSegments
}

Trace.prototype.getCollectedChildren = function getCollectedChildren(id) {
  const collectedChildren = this.segments.filter((segment) => segment.parentId === id && segment._collect && !segment.ignore)
