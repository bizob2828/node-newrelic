 ...
 * @param config.config
 * @param config.name
 * @param config.collect
 * @param config.traceStacks
 * @param config.root
 * @param config.isRoot
 ...
 *  'mysql', etc).
 */
function TraceSegment({ config, name, collect, traceStacks, parentId, root, isRoot = false }) {
  this.isRoot = isRoot
  if (isRoot) {
...
TraceSegment.prototype.probe = function probe(action) {
  ...
  root.traceStacks.probe(action, { segment: this.name })
}
...
TraceSegment.prototype.end = function end() {
  ...
  this.probe('Ended', { segment: this.name })
  this.timer.end()
  this._updateRootTimer()
}
...
TraceSegment.prototype.add = function add({ config, name, collect, root }) {
  ...
  return new TraceSegment({ config, name, collect, root, parentId: this.id })
}
