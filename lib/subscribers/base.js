...
class Subscriber {
  ...
  insertDTHeaders({ ctx, headers }) {
    ...
    // All callers of `insertDTHeaders` are message queue producers, so flag this
    // as a message queue call to enable the `nrns` not-sampled header behavior.
    tx.insertDistributedTraceHeaders(
      headers,
      null,
      { traceId: tx.traceId, spanId: ctx?.segment?.id, traceFlags },
      { isMessageQueue: true }
    )
  }
