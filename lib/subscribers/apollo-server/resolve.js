...
const securityChannels = require('#agentlib/security-channels.js')
...
module.exports = class ApolloResolveSubscriber extends BaseSubscriber {
  ...
  wrapResolve(origResolve, thisArg, args) {
    ...
    if (securityChannels.graphqlResolveStart.hasSubscribers && transaction) {
      securityChannels.graphqlResolveStart.publish({
        query: info.operation?.loc?.source?.body || null,
        operationType: info.operation?.operation || null,
        operationName: info.operation?.name?.value || null,
        variables: resolverArgs || null,
        transaction,
